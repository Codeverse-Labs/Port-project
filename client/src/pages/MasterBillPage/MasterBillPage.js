...
function MasterBillPage() {
  ...
  const { data: departmentData, isLoading } = useGetDepartmentBillsQuery({month, year, department});
  ...
  return (
    <>
      ...
      <div className=" flex flex-row">
        ...
        {(department !== "All" && department !== '') && !isLoading && departmentData?.length !== 0 &&
          ...
          <table className="table-auto border-collapse w-full mx-10 mt-6">
            ...
            <tbody>
              {departmentData?.map((row, rowIndex) => (
                <tr key={rowIndex}>
                  {Object.keys(departmentData[0]).map((header, colIndex) => (
                    <td
                      key={colIndex}
        ...
        {!isLoading && departmentData?.length === 0 && (department !== "All" && department !== '') &&
        <div className= "flex-grow justify-center justify-items-center">
        <div className="mt-10 font-bold text-2xl text-center">
        No records available 
        </div>
        </div>}
        {(department === "All" || department === '')  && <MonthlyBillPage month={month} year={year}/>}
      </div>
    </>
  );
};
